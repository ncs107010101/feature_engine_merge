...
> **總計 343 個特徵**，分佈在 9 個主要模組中（Alpha v7 新增 25 個，Alpha v8 新增 3 個，Alpha v9 新增 3 個，Alpha v10 新增 4 個，Alpha v11 新增 3 個，Alpha v12 新增 4 個，Alpha v13 新增 13 個，Alpha v14 新增 5 個，Alpha v15 新增 7 個，Alpha v16 新增 5 個，Alpha v17 新增 26 個，Alpha v18 新增 4 個，NLD 非線性動力學新增 15 個，AFD cross_tick_daily 新增 10 個）
...
| # | 特徵名稱 | 計算方式 | 使用欄位 | 算子 | 邏輯說明 |
|---|---------|---------|---------|------|---------|
| 1 | `f_nld_attractor_change_of_mind` | 早盤(前30分) vs 尾盤(後30分) 的 (ΔP, OFI) 向量外積 | `DealPrice`, `DealCount`, `PrFlag`, `DealTimeSecond` | cross_product_2d, zscore_rolling(20) | **吸引子網路決策逆轉**：早盤與尾盤的價格變化與訂單流不平衡的向量外積，衡量市場方向性逆轉強度。 |
| 2 | `f_nld_cell_fate_logic_phase` | arctan2(d²V_sell/dt², d²V_buy/dt²) → cos(φ) × AsymSign | `DealCount`, `PrFlag`, `DealPrice` | safe_second_derivative, zscore_rolling(20) | **細胞命運邏輯相位**：買賣成交量二階導數的相位角餘弦，衡量多空加速度的非對稱性。 |
| 3 | `f_nld_coupled_oscillator_rotation` | (dBuyPr/dt, dSellPr/dt, dDealPrice/dt) 3D 外積 | `BuyPr`, `SellPr`, `DealPrice` | diff, zscore_rolling(20) | **耦合振盪器旋轉**：買賣報價與成交價的三維速度外積，捕捉市場微觀結構的旋轉動力學。 |
| 4 | `f_nld_dpli_buyer_seller_lead` | 買/賣 DealCount 差分 → arctan2 近似相位 → 相位差 d² × AsymSign | `DealCount`, `PrFlag`, `DealPrice`, `DealTimeSecond` | diff, arctan2, zscore_rolling(20) | **dPLI買賣相位領先**：買賣成交量的相位差二階導數，衡量多空輪動的角加速度。 |
| 5 | `f_nld_ei_directed_phase_cross` | E/I 成交量 10s bin → 相位角 → 2D 外積 | `DealCount`, `PrFlag`, `DealTimeSecond` | diff, arctan2, cross_product_2d, zscore_rolling(20) | **E/I有向相位外積**：興奮性/抑制性成交量的相位角外積，衡量多空動力的有向耦合。 |
| 6 | `f_nld_ei_transition_eigen_dot` | PrFlag 構建 E/I 2×2 轉移矩陣 → 主特徵向量 · [ΔP_E, ΔP_I] | `DealPrice`, `PrFlag` | build_transition_matrix, stationary_distribution, zscore_rolling(20) | **E/I轉移特徵內積**：E/I狀態轉移矩陣的平穩分佈與價格位移的內積，衡量狀態機與價格動量的對齊。 |
| 7 | `f_nld_gamma_beta_cross_phase` | 1s/10s VWAP 變動相位差 → d² → × sign(net_ofi) | `DealPrice`, `DealCount`, `PrFlag`, `DealTimeSecond` | VWAP, diff, arctan2, zscore_rolling(20) | **Gamma-Beta跨頻相位耦合**：1秒與10秒VWAP變動的相位差二階導，衡量高頻與低頻成分的相位耦合崩潰。 |
| 8 | `f_nld_inhibitory_collapse_accel` | C_EI = I_Vol/(E_Vol+ε) → log → d² → 負加速 × sign(ΔP) | `DealCount`, `PrFlag`, `DealPrice`, `DealTimeSecond` | safe_second_derivative, zscore_rolling(20) | **抑制性崩潰加速度**：E/I成交量比率的對數二階導，衡量賣壓相對於買壓的加速崩潰。 |
| 9 | `f_nld_inhibitory_pump_exhaust` | 反向抵抗量(上漲時PrFlag=0量) → d² → min(0,d²) × sign(ΔP) | `DealPrice`, `DealCount`, `PrFlag` | diff, safe_second_derivative, zscore_rolling(20) | **抑制性幫浦耗竭**：反向抵抗量的二階導負值，衡量市場抵抗力的加速衰竭。 |
| 10 | `f_nld_prc_stimulus_reset` | 大單(>95th)打入時 d²VWAP/dt² × sign(大單方向) 條件觸發型 | `DealPrice`, `DealCount`, `PrFlag` | VWAP, safe_second_derivative, zscore_rolling(20) | **PRC相位重置**：大單觸發時的VWAP加速度，衡量大額成交對市場節奏的重置效應。 |
| 11 | `f_nld_rna_velocity_splicing` | 報價動能 (dBuyPr, dSellPr) × 成交動能 (ActBuyVol, ActSellVol) 外積 → d² | `BuyPr`, `SellPr`, `DealCount`, `PrFlag`, `DealTimeSecond` | cross_product_2d, safe_second_derivative, zscore_rolling(20) | **RNA速度剪接**：報價動能與成交動能的外積二階導，衡量意圖與執行的動態差異。 |
| 12 | `f_nld_synaptic_decay_cross_dot` | E快速平滑 vs I慢速平滑 → 各自二階導 → 內積 × AsymSign | `DealCount`, `PrFlag`, `DealPrice` | ewm, safe_second_derivative, zscore_rolling(20) | **突觸衰減交叉內積**：快/慢EWM平滑的二階導內積，衡量多空動態的時間尺度耦合。 |
| 13 | `f_nld_bifurcation_delay_eigen` | 15分鐘bin (ΔP, ΔV) 共變異數 → 特徵值差距 → 最小gap時加速度 × AsymSign | `DealPrice`, `DealCount`, `DealTimeSecond` | eigenvalue_gap, zscore_rolling(20) | **分岔延遲特徵加速度**：15分鐘時間區間內價格與成交量變化的共變異數矩陣特徵值差距最小時的加速度，衡量對稱吸引子附近的分岔延遲現象。 |
| 14 | `f_nld_bimodal_eigen_proj` | 5分鐘bin價格雙峰檢測(bins=50, order=3) → 2×2轉移矩陣 → 特徵向量 · [accel_high, accel_low] | `DealPrice`, `DealTimeSecond` | histogram(bins=50), gaussian_filter(sigma=2), argrelextrema(order=3), zscore_rolling(20) | **雙模態特徵投影**：5分鐘價格區間的雙峰檢測（50bins, order=3），透過轉移矩陣特徵向量與高低區域加速度的內積，衡量雙穩態系統的動態投影。 |
| 15 | `f_nld_critical_slowing_phase_accel` | 100-tick bin AC(1) → arctan2(vol, AC) → 相位差d² × OFIsign | `DealPrice`, `DealCount`, `PrFlag` | autocorr, arctan2, zscore_rolling(20) | **臨界減速相位加速度**：100-tick區間的價格變化自相關與成交量的相位角二階導，衡量臨界轉變前的減速現象。 |
| 16 | `f_nld_dtb_singularity_cross` | dp_s(EWM span=5)與ddp 3D速度外積 → Z分量(cz=x1·y2−y1·x2) → 加權平均 | `DealPrice`, `DealCount` | ewm(span=5), cross_product_3d_z, zscore_rolling(20) | **DTB奇點穿越**：價格變化與其二階導數的3D速度外積Z分量（非3D norm），衡量degenerate Takens-Bogdanov奇點附近的穿越行為。 |
| 17 | `f_nld_hopf_eigen_asymmetry` | dp三態轉移矩陣 → 最大實特徵值 × (tm[0,0]−tm[2,2]) | `DealPrice` | price_to_three_states, transition_matrix, zscore_rolling(20) | **Hopf特徵非對稱**：價格變化三態轉移矩陣的主特徵值與狀態0/2轉移機率差的乘積，衡量Hopf分岔附近的特徵非對稱性。 |
| 18 | `f_nld_hysteresis_area` | 正規化(dp, dv) 50-tick bin → 有向面積 → 平均 | `DealPrice`, `DealCount` | cross_product_2d, zscore_rolling(20) | **滯後環面積**：正規化價格與成交量變化的有向面積，衡量滯後現象的強度。 |
| 19 | `f_nld_limit_cycle_collapse_markov` | dp三態轉移矩陣 → 平穩分佈 · [1, 0, −1] | `DealPrice` | transition_matrix, stationary_dist, zscore_rolling(20) | **極限環崩潰Markov**：價格變化三態轉移矩陣的平穩分佈投影，衡量極限環崩潰後的Markov狀態分佈。 |
| 20 | `f_nld_limit_cycle_radius_accel` | R=sqrt(dp²+dv²) EWM(span=10) → d²R × AsymSign | `DealPrice`, `DealCount` | ewm(span=10), zscore_rolling(20) | **極限環半徑加速度**：價格與成交量變化構成的極座標半徑的二階導，衡量極限環的膨脹或收縮加速度。 |
| 21 | `f_nld_lyapunov_divergence_accel` | 延遲嵌入(k=10)距離D EWM(span=10) → d²D × AsymSign | `DealPrice`, `DealTimeSecond` | delay_embedding, ewm(span=10), zscore_rolling(20) | **Lyapunov發散加速度**：價格延遲嵌入空間的軌跡發散度二階導，衡量混沌系統的發散加速度。 |
| 22 | `f_nld_neimark_sacker_torus` | VWAP/dp/dv 3D外積norm → d² × AsymSign | `DealPrice`, `DealCount` | cross_product_3d, zscore_rolling(20) | **Neimark-Sacker環面**：VWAP、價格變化、成交量變化構成的3D外積二階導，衡量環面分岔的相位耦合。 |
| 23 | `f_nld_saddle_loop_divergence` | 價格-VWAP穿越間隔 → 二階差分 × 最後偏離sign | `DealPrice`, `DealCount` | crossing_intervals, zscore_rolling(20) | **鞍結環發散**：價格圍繞VWAP穿越的時間間隔二階差分，衡量鞍結環附近的發散行為。 |
| 24 | `f_nld_saddle_node_phase_accel` | arctan2(sellPr-price, price-buyPr) EWM(span=10) → d²φ最大值 | `DealPrice`, `BuyPr`, `SellPr` | arctan2, ewm(span=10), zscore_rolling(20) | **鞍結點相位加速度**：買賣報價相對於成交價的相位角二階導，衡量鞍結點附近的相位加速度。 |
| 25 | `f_nld_stochastic_bifurcation_phase` | 價格雙峰檢測(bins=50, order=3) → VWAP半區間加速度 → mode_vec · accel_vec | `DealPrice`, `DealCount` | histogram(bins=50), gaussian_filter(sigma=2), argrelextrema(order=3), zscore_rolling(20) | **隨機分岔相位**：價格雙峰檢測（50bins, order=3）後，VWAP前半與後半區間加速度與模態向量的內積，衡量隨機分岔的相位特性。 |
| 26 | `f_nld_stochastic_focusing_phase` | arctan2(dp, dv) EWM(span=10) → d²θ × AsymSign | `DealPrice`, `DealCount` | arctan2, ewm(span=10), zscore_rolling(20) | **隨機聚焦相位**：價格與成交量變化的相位角二階導，衡量隨機聚焦的相位特性。 |
| 27 | `f_nld_transient_attractor_proj` | d²Price EWM(span=10) × 吸引子軸距離正規化 | `DealPrice`, `CeilPr`, `FloorPr` | ewm(span=10), zscore_rolling(20) | **瞬態吸引子投影**：價格二階導與漲跌停距離正規化的內積，衡量瞬態吸引子的投影強度。 |

---

## 十四、Alpha v14 生態模擬特徵（5 個特徵）
...
## 二十二、AFD 大氣流體力學特徵（交叉逐筆日頻，10 個特徵）
...
| # | 特徵名稱 | 計算方式 | 使用欄位 | 算子 | 邏輯說明 |
|---|---------|---------|---------|------|---------|
| 1 | `f_afd_cold_sector_convective_trigger` | cold_zone_buy × cold_slope → zscore_rolling(20) | `raw_cold_zone_buy`, `raw_cold_slope` | zscore_rolling(20, eps=1e-10) | **冷區對流加熱**：冷區（均價以下）主動買量與冷區斜率的乘積，代表資金在低價區集結推動價格上漲的對流效應。 |
| 2 | `f_afd_latent_heat_waterfall_slope` | 融券餘額 × tick_slope (當 both > 0) → zscore_rolling(42) | `raw_margin_balance`, `tick_slope` | zscore_rolling(42, eps=1e-10) | **融券槓桿點火**：融券餘額與正向價格斜率同時存在時觸發，代表空頭回補動能點燃上漲趨勢。 |
| 3 | `f_afd_baroclinic_ageostrophic_div` | max(0, close_p - vwap) × total_vol → log1p → zscore_rolling(20) | `close_pr`, `vwap`, `total_vol` | zscore_rolling(20, eps=1e-10) | **前沿價格脫離重心**：價格偏離日內均值的程度與成交量之積，代表脫離均衡態的發散強度。 |
| 4 | `f_afd_pv_pt_resonance_jacobian` | det(vwap_slope × tick_slope) × max(0, close_p - vwap) → zscore_rolling(42) | `vwap`, `tick_slope`, `close_pr` | zscore_rolling(42, eps=1e-10) | **宏觀微觀斜率共振**：VWAP 宏觀趨勢與 tick 微觀趨勢的 Jacobian 行列式，捕捉多尺度共振效應。 |
| 5 | `f_afd_storm_relative_helicity` | (tick_mean_price - vwap) × 5日報酬滾動和 → zscore_rolling(42) | `raw_tick_mean_price`, `vwap`, `報酬率` | zscore_rolling(42, eps=1e-10) | **風暴相對螺旋度**：價格相對重心與環境報酬的外積，捕捉價格旋轉動能。 |
| 6 | `f_afd_helical_turbulence_filter` | (large_buy / (small_buy + 1)) × max(0, close_p - open_p) → log1p → zscore_rolling(42) | `_large_buy`, `_small_buy`, `close_pr`, `open_pr` | zscore_rolling(42, eps=1e-10) | **高螺旋度濾波**：機構大單與散單的信雜比與正向價格變動之積，代表動能過濾雜訊後的強度。 |
| 7 | `f_afd_occlusion_zone_vorticity` | (above_vwap_buy - above_vwap_high_buy) × (close_p - open_p) → zscore_rolling(20) | `above_vwap_buy`, `above_vwap_high_buy`, `close_pr`, `open_pr` | zscore_rolling(20, eps=1e-10) | **錮囚區渦度**：廣大買盤支撐與高價區賣壓的差值與價格變動之積，代表多空交戰區的旋轉效應。 |
| 8 | `f_afd_ground_relative_friction_shear` | market_ret × (stock_ret - market_ret) × vol → zscore_rolling(20) | `報酬率`, `total_vol` | zscore_rolling(20, eps=1e-10) | **大盤摩擦剪切**：個股相對大盤的超額報酬與成交量之積，捕捉個股與大盤的相對摩擦效應。 |
| 9 | `f_afd_optimal_drag_coefficient` | Gaussian_kernel(drag_ratio) × max(0, slope) → zscore_rolling(42) | `raw_drag_ratio`, `raw_tick_slope` | zscore_rolling(42, eps=1e-10) | **最佳阻力係數**：基於 20 日中位數的高斯核權重與正向斜率之積，篩選最優價格阻力位置。 |
| 10 | `f_afd_friction_riverbend_exchange` | spread_shift × net_buy × positive_deltaP (50-tick bins) → zscore_rolling(20) | `raw_riverbend_val` | zscore_rolling(20, eps=1e-10) | **河彎效應**：價格流過彎道（VWAP 交叉）的交換動力學，代表流體摩擦耗散效應。 |

**共享中間變數** (`preprocess_cross_tick_daily` + `_compute_afd_intermediates` + `_compute_afd_tick_level_intermediates`):
- 基礎 tick 變數：`total_vol`, `buy_vol`, `sell_vol`, `open_pr`, `close_pr`, `vwap`
...
- 日頻變數：`收盤價`, `開盤價`, `報酬率`, `融券餘額(張)`
- 重要：所有 AFD 特徵使用 `eps=1e-10` 以確保數值穩定性

---

## 二十三、AFD v2 大氣流體力學特徵（交叉券商分點逐筆，15 個特徵）
...
| # | 特徵名稱 | 計算方式 | 使用欄位 | 算子 | 邏輯說明 |
|---|---------|---------|---------|------|---------|
| 1 | `f_afd_moist_cisk_feedback` | det(cov(ret,Top10_NetBuy)) × max(0,Top10-Retail) → log1p → Z(42) | `raw_tick_ret`, `raw_top10_net_buy`, `raw_retail_net_buy` | rolling_cov(5), zscore_rolling(42, eps=1e-10) | **CISK 反饋**：5日滾動協方差矩陣行列式×大戶淨買超額，捕捉跨週期共振 |
| 2 | `f_afd_diabatic_pv_asymmetric` | max(0,j_buy.diff2)×ret - min(0,j_sell.diff2)×ret → Z(20) | `raw_top10_net_buy`, `raw_retail_net_buy`, `raw_tick_ret` | diff(2), zscore_rolling(20, eps=1e-10) | **非絕熱 PV 非對稱**：券商買超二階差分與價格報酬的非對稱組合 |
| 3 | `f_afd_jet_exit_radiation` | max(0,Top5_NetBuy) × ret.diff() → Z(20) | `raw_top5_net_buy`, `raw_tick_ret` | diff, zscore_rolling(20, eps=1e-10) | **噴流輻射**：大戶淨買與價格加速度的乘積效應 |
| 4 | `f_afd_edge_wave_phase_locking` | (Top10_5d_mean × Retail_5d_mean) × sign(ret) → log1p → Z(20) | `raw_top10_net_buy`, `raw_retail_net_buy`, `raw_tick_ret` | rolling(5).mean, zscore_rolling(20, eps=1e-10) | **邊緣波相位鎖定**：大戶與散戶動量共振，同向為正、背離為負 |
| 5 | `f_afd_optimal_growth_hindering` | min(0,buyVol-sellVol) × delta_p × sign(delta_p) → Z(20) | `raw_buy_vol`, `raw_sell_vol`, `raw_delta_p` | zscore_rolling(20, eps=1e-10) | **最佳增長阻礙**：空頭動能被吸收的軋空預測器 |
| 6 | `f_afd_nlsv_energy_extraction` | det(slope_matrix) × max(0,delta_p) → Z(42) | `raw_top10_net_buy_slope_5d`, `raw_top10_net_buy_slope_20d`, `raw_ret_slope_5d`, `raw_ret_slope_20d`, `raw_delta_p` | rolling_slope, zscore_rolling(42, eps=1e-10) | **NLSV 能量提取**：大戶淨買與報酬能量提取矩陣行列式 |
| 7 | `f_afd_entrainment_mass_flux` | max(0,delta_p) × exp(-clip(epsilon, -5, 5)) → Z(42) | `raw_top10_net_buy`, `raw_delta_p` | diff(5), zscore_rolling(42, eps=1e-10) | **夾帶質量通量**：entrainment rate trigger，指數衰減與價格變化 |
| 8 | `f_afd_dynamic_vppgf_asymmetry` | max(0,large²-small²) × delta_p → log1p → Z(42) | `raw_top10_net_buy`, `raw_retail_net_buy`, `raw_delta_p` | zscore_rolling(42, eps=1e-10) | **動態 VP/PGF 非對稱**：大戶動量平方與散戶動量平方的差異 |
| 9 | `f_afd_cold_pool_buoyancy_gradient` | max(0,u_dyn-b_cold) × sign(delta_p) → Z(20) | `raw_top5_net_buy`, `raw_retail_net_buy`, `raw_delta_p` | zscore_rolling(20, eps=1e-10) | **冷池浮力梯度**：大戶動力抬升克服散戶負浮力的程度 |
| 10 | `f_afd_goldilocks_cold_pool` | g_c × u_draft × sign(delta_p) → log1p → Z(42) | `raw_retail_net_buy`, `raw_top5_net_buy`, `raw_delta_p` | rolling(5).sum, rolling(20).quantile(0.75), zscore_rolling(42, eps=1e-10) | **適居帶冷池**：適度回檔後噴發的信號 |
| 11 | `f_afd_updraft_width_entrainment` | NB_BuyBrokers × max(0,delta_p) → Z(20) | `NB_BuyBrokers`, `raw_delta_p` | zscore_rolling(20, eps=1e-10) | **上升氣流寬度夾帶**：買方券商數量與正價格變化的乘積 |
| 12 | `f_afd_svc_baroclinic_stretching` | max(0,a_flow.diff2 × omega_s) × max(0,delta_p) → Z(20) | `raw_buy_vol`, `raw_total_vol_tick`, `raw_delta_p` | diff(2), zscore_rolling(20, eps=1e-10) | **SVC 斜壓拉伸**：買盤加速度與主動買流向的組合效應 |
| 13 | `f_afd_random_vorticity_stretching` | Jacobian(v_small,v_large) × max(0,delta_p) → Z(42) | `raw_small_buy`, `raw_small_sell`, `raw_large_buy`, `raw_large_sell`, `raw_delta_p` | diff, zscore_rolling(42, eps=1e-10) | **隨機渦度拉伸**：散戶與大戶的拓撲拉伸 Jacobian |
| 14 | `f_afd_surface_drag_convergence` | max(0,spread.diff) × max(0,a_price) → Z(20) | `raw_spread_mean`, `raw_a_price` | diff, zscore_rolling(20, eps=1e-10) | **表面曳力收斂**：價差梯度與價格加速度的正值乘積 |
| 15 | `f_afd_rfd_occlusion_wrap_around` | max(0,sell_z) × buyVol × sign(delta_p) → log1p → Z(42) | `raw_sell_vol`, `raw_buy_vol`, `raw_delta_p` | zscore_rolling, zscore_rolling(42, eps=1e-10) | **RFD 包覆繞行**：砸盤後正交反轉的信號 |

**共享中間變數** (`preprocess_cross_broker_tick` + 新增):
- 券商分點變數：`raw_top10_net_buy`, `raw_top5_net_buy`, `raw_retail_net_buy`, `NB_BuyBrokers`
- 逐筆成交變數：`raw_tick_ret`, `raw_delta_p`, `raw_buy_vol`, `raw_sell_vol`, `raw_total_vol_tick`
- 大小單變數：`raw_large_buy`, `raw_small_buy`, `raw_large_sell`, `raw_small_sell`
- 價格變數：`raw_spread_mean`, `raw_a_price`
- 斜率變數：`raw_top10_net_buy_slope_5d`, `raw_top10_net_buy_slope_20d`, `raw_ret_slope_5d`, `raw_ret_slope_20d`
- 重要：所有 AFD v2 特徵使用 `eps=1e-10` 以確保數值穩定性

---

## AFD 特徵分析屬性（2025 新增）

> 本節記錄 AFD 大氣流體力學特徵的分析屬性，供模型訓練與特徵選擇參考。

### A. 未來資料使用分析 (Look-Ahead Bias)

**結論：所有 AFD 特徵均未使用未來資料，不存在 Look-Ahead Bias。**

| 分析項目 | 結果 | 說明 |
|----------|------|------|
| `zscore_rolling` | ✅ 安全 | 使用 pandas `.rolling(window)`，嚴格僅參考歷史資料 |
| `diff()` / `pct_change()` | ✅ 安全 | 僅使用 `shift(1)` 及以上的歷史資料 |
| `shift(-1)` | ❌ 未使用 | 整個 AFD 模組中未發現任何未來資料引用 |

**潛在 Same-Day Bias**：zscore_rolling 計算時包含當日觀測值（`min_periods` 允許窗口內計算），這在日頻資料中可接受，但若用於日內即時計算需注意。

### B. 橫斷面比較適用性

| 特徵名 | 橫斷面適用性 | 原因 |
|--------|--------------|------|
| `f_afd_updraft_width_entrainment` (F36) | ⭐ 最佳 | `NB_BuyBrokers` 為券商數量計數，與市值無關 |
| `f_afd_surface_drag_convergence` (F39) | ⭐ 佳 | `spread.diff` 與 `a_price` 為相對比率 |
| 其他大戶淨買特徵 (F27-F35) | ❌ 不適合 | `Top10_NetBuy`, `Top5_NetBuy` 為絕對金額，受市值影響 |
| 成交量相關特徵 | ❌ 不適合 | `TotalBuy`, `total_vol` 與市值高度相關 |

**建議**：橫斷面比較時優先使用 F36、F39，或自行進行市值標準化。

### C. 尺規不變性 (Scale Invariance)

**結論：zscore_rolling + clip(-5, 5) 實現了「個股內」尺規不變性，但「跨股票」比較需注意。**

| 特性 | 說明 |
|------|------|
| z-score 計算範圍 | 僅在 `StockId` 內進行滾動標準化 |
| clip 邊界 | [-5, 5]，將極端值截斷至常見範圍 |
| 跨股票可比性 | **不相同**：不同股票的 z-score 分佈形態不同，無法直接橫向比較 |
| 正確橫向比較方式 | 需額外使用 `groupby('Date').rank(pct=True)` 進行橫斷面排名 |

### D. CatBoost LTR 任務特徵建議

**結論：建議同時保留 Z-Score 特徵與橫斷面排名特徵，兩者提供互補訊號。**

| 特徵類型 | 生成方式 | 訊號性質 |
|----------|----------|----------|
| Z-Score 特徵 | `zscore_rolling(x, window)` + `clip(-5, 5)` | 捕捉個股相對於自身歷史的異常程度 |
| 橫斷面排名 | `groupby('Date')[x].rank(pct=True)` | 捕捉個股當日在所有股票中的相對位階 |

**互補性**：
- Z-Score：適合捕捉個股「趨勢異常」（如連續大戶買超偏離歷史均值）
- 橫斷面排名：適合捕捉「相對強度」（如今日大戶買入力度在全部股票中的排名）
