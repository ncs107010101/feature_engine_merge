...
| `Alpha AFD – single_stock_tick (25)` | 大氣流體力學特徵 (Tick) | 25 | DRV非絕熱轉換、相位鎖定、Jacobian行列式、渦度流、臨界分岔 |
...
## 二十六、Alpha AFD 大氣流體力學特徵（新增，逐筆，25個）
...
| # | 特徵名 | 計算方式 | 概念邏輯 |
|---|--------|---------|---------|
| 1 | `f_afd_diabatic_conversion_cross` | `Σ(V × max(0, ΔP) - V × |ΔP|)` → rolling_zscore(20) | **DRV非絕熱加熱轉換率**：外積衡量主動單方向與價格位移正交時的「潛熱釋放」，區分真假突破 |
| 2 | `f_afd_latent_heat_phase_locking` | BIN_SIZE=30, Σ(inner_prod × trigger)` → rolling_zscore(20) | **DRV低層正渦度與中層負渦度的相位鎖定**：捕捉買賣單在時間序列上的瞬時同步性 |
| 3 | `f_afd_condensation_burst_asymmetry` | `Σ(C_t × DealCount)³` → log → rolling_zscore(20) | **降水凝結引發的位渦相變奇點**：三次方運算子放大向上掃盤的極端非對稱方向 |
| 4 | `f_afd_continuous_spectrum_transient` | `alignment × volume` → log → rolling_zscore(20) | **擾動投射到連續譜的瞬間非模態增長**：超高頻與長週期趨勢的瞬間對齊 |
| 5 | `f_afd_crosswise_to_streamwise` | `Σ(V_cross × V_stream × trigger)` → rolling_zscore(20) | **河彎效應中橫向渦度向流向渦度的交換**：掛單不平衡 × 主動買量 × 正向價格 |
| 6 | `f_afd_moist_baroclinic_acceleration` | `(E_m_late - E_d_late) / (E_m_early + E_d_early)` → rolling_zscore(20) | **濕對流斜壓加速度**：濕對流環境中的斜壓性加速效應 |
| 7 | `f_afd_moist_enstrophy_jacobian` | BIN_SIZE=50, Σ(Jacobian)` → rolling_zscore(20) | **濕對流渦度擬能Jacobian**：Jacobian行列式捕捉渦度與價格的耦合 |
| 8 | `f_afd_lc1_frontal_gravity_wave` | `Σ(stream × cross)` → rolling_zscore(20) | **LC1鋒面重力波**：主動單方向與價格變動的交叉效應 |
| 9 | `f_afd_lc2_cyclonic_breaking` | BIN_SIZE=100, `max(cumsum) - min(cumsum)` → rolling_zscore(20) | **LC2氣旋breaking**：累積方向的範圍捕捉氣旋breakin現象 |
| 10 | `f_afd_tropopause_pv_intrusion` | `(early × mid)` → rolling_zscore(20) | **對流層頂位渦侵入**：三段式時間分割的early-mid交互 |
| 11 | `f_afd_vortex_dipole_ejection` | `(am × pm)` → rolling_zscore(20) | **渦偶極子射出**：上午與下午方向流的乘積效應 |
| 12 | `f_afd_vortex_stretching_dot` | `Σ(vortex × price_diff)` → rolling_zscore(20) | **渦管拉伸點積**：渦管方向與價格變動的內積效應 |
| 13 | `f_afd_vortex_line_slippage` | `(late - mid) × early` → rolling_zscore(20) | **渦線滑移**：三段式時間分割的滑移效應 |
| 14 | `f_afd_nlsv_meridional_elongation` | BIN_SIZE=100, `Σ(|J|)` → rolling_zscore(20) | **NLSV經向延伸**：經向方向的累積擴展效應 |
| 15 | `f_afd_nlsv_meridional_extension_tensor` | `(first_half × second_half)` → rolling_zscore(20) | **NLSV經向擴展張量**：前後半段方向流的乘積效應 |
| 16 | `f_afd_nlsv_zonal_shear_alignment` | BIN_SIZE=100, `Σ(A × B)` → rolling_zscore(20) | **NLSV帶狀剪切對齊**：帶狀剪切與方向流的對齊效應 |
| 17 | `f_afd_pv_unshielding_orr` | `autocorr_sum(direction)` → rolling_zscore(20) | **位渦去屏蔽Orr機制**：方向流的自相關效應捕捉Orr機制 |
| 18 | `f_afd_streamfunction_variance_filter` | `var(cumsum(direction))` → rolling_zscore(20) | **流函數方差濾波**：累積方向流的方差效應 |
| 19 | `f_afd_sv_optimization_time_divergence` | `(mean(second) - mean(first))` → rolling_zscore(20) | **流函數優化時間發散**：前後半段平均方向流的發散效應 |
| 20 | `f_afd_optimal_perturbation_jacobian` | BIN_SIZE=50, `Σ(Jacobian)` → rolling_zscore(20) | **最優擾動Jacobian**：最優擾動理論的Jacobian行列式效應 |
| 21 | `f_afd_vorticity_river_tilting_jacobian` | `Σ(Jacobian)` → rolling_zscore(20) | **渦度河流傾斜Jacobian**：河流效應的傾斜Jacobian效應 |
| 22 | `f_afd_wave_mean_flow_jacobian` | `Σ(Jacobian)` → rolling_zscore(20) | **波-平均流Jacobian**：波動與平均流的Jacobian交叉效應 |
| 23 | `f_afd_lfcb_convergence_tensor` | BIN_SIZE=100, `Σ(flow_i × flow_i+1)` → rolling_zscore(20) | **LFCB收斂張量**：相鄰時間區塊的收斂效應 |
| 24 | `f_afd_bred_vector_shear_penalty` | 60/40 split, `cross × min(0, dot) × sign` → log → rolling_zscore(42) | **繁殖向量的負向剪切**：前段趨勢與後段擾動的幾何交叉 |
| 25 | `f_afd_bifurcation_tipping_point` | BIN_SIZE=200, threshold=mean+2σ → rolling_zscore(42) | **臨界分岔點**：系統能量突變時的方向性觸發 |
