...
| `single_stock_tick (NLD)` | 非線性動力學特徵 (Tick) | 15 | 分岔延遲、雙模態投影、臨界減速、DTB奇點、Hopf分岔、滯後環、極限環、Lyapunov發散、Neimark-Sacker環面、鞍結環、鞍結點、隨機分岔、隨機聚焦、瞬態吸引子 |
...
| # | 特徵名 | 計算方式 | 概念邏輯 |
|---|--------|---------|---------|
| 1 | `f_nld_attractor_change_of_mind` | 早盤(前30分) vs 尾盤(後30分) 的 (ΔP, OFI) 向量外積 | **吸引子網路決策逆轉**：早盤與尾盤的價格變化與訂單流不平衡的向量外積，衡量市場方向性逆轉強度。 |
| 2 | `f_nld_cell_fate_logic_phase` | arctan2(d²V_sell/dt², d²V_buy/dt²) → cos(φ) × AsymSign | **細胞命運邏輯相位**：買賣成交量二階導數的相位角餘弦，衡量多空加速度的非對稱性。 |
| 3 | `f_nld_coupled_oscillator_rotation` | (dBuyPr/dt, dSellPr/dt, dDealPrice/dt) 3D 外積 | **耦合振盪器旋轉**：買賣報價與成交價的三維速度外積，捕捉市場微觀結構的旋轉動力學。 |
| 4 | `f_nld_dpli_buyer_seller_lead` | 買/賣 DealCount 差分 → arctan2 近似相位 → 相位差 d² × AsymSign | **dPLI買賣相位領先**：買賣成交量的相位差二階導數，衡量多空輪動的角加速度。 |
| 5 | `f_nld_ei_directed_phase_cross` | E/I 成交量 10s bin → 相位角 → 2D 外積 | **E/I有向相位外積**：興奮性/抑制性成交量的相位角外積，衡量多空動力的有向耦合。 |
| 6 | `f_nld_ei_transition_eigen_dot` | PrFlag 構建 E/I 2×2 轉移矩陣 → 主特徵向量 · [ΔP_E, ΔP_I] | **E/I轉移特徵內積**：E/I狀態轉移矩陣的平穩分佈與價格位移的內積，衡量狀態機與價格動量的對齊。 |
| 7 | `f_nld_gamma_beta_cross_phase` | 1s/10s VWAP 變動相位差 → d² → × sign(net_ofi) | **Gamma-Beta跨頻相位耦合**：1秒與10秒VWAP變動的相位差二階導，衡量高頻與低頻成分的相位耦合崩潰。 |
| 8 | `f_nld_inhibitory_collapse_accel` | C_EI = I_Vol/(E_Vol+ε) → log → d² → 負加速 × sign(ΔP) | **抑制性崩潰加速度**：E/I成交量比率的對數二階導，衡量賣壓相對於買壓的加速崩潰。 |
| 9 | `f_nld_inhibitory_pump_exhaust` | 反向抵抗量(上漲時PrFlag=0量) → d² → min(0,d²) × sign(ΔP) | **抑制性幫浦耗竭**：反向抵抗量的二階導負值，衡量市場抵抗力的加速衰竭。 |
| 10 | `f_nld_prc_stimulus_reset` | 大單(>95th)打入時 d²VWAP/dt² × sign(大單方向) 條件觸發型 | **PRC相位重置**：大單觸發時的VWAP加速度，衡量大額成交對市場節奏的重置效應。 |
| 11 | `f_nld_rna_velocity_splicing` | 報價動能 (dBuyPr, dSellPr) × 成交動能 (ActBuyVol, ActSellVol) 外積 → d² | **RNA速度剪接**：報價動能與成交動能的外積二階導，衡量意圖與執行的動態差異。 |
| 12 | `f_nld_synaptic_decay_cross_dot` | E快速平滑 vs I慢速平滑 → 各自二階導 → 內積 × AsymSign | **突觸衰減交叉內積**：快/慢EWM平滑的二階導內積，衡量多空動態的時間尺度耦合。 |
| 13 | `f_nld_bifurcation_delay_eigen` | 15分鐘bin (ΔP, ΔV) 共變異數 → 特徵值差距 → 最小gap時加速度 × AsymSign | **分岔延遲特徵加速度**：15分鐘時間區間內價格與成交量變化的共變異數矩陣特徵值差距最小時的加速度，衡量對稱吸引子附近的分岔延遲現象。 |
| 14 | `f_nld_bimodal_eigen_proj` | 5分鐘bin價格雙峰檢測(bins=50, order=3) → 2×2轉移矩陣 → 特徵向量 · [accel_high, accel_low] | **雙模態特徵投影**：5分鐘價格區間的雙峰檢測（50bins, order=3），透過轉移矩陣特徵向量與高低區域加速度的內積，衡量雙穩態系統的動態投影。 |
| 15 | `f_nld_critical_slowing_phase_accel` | 100-tick bin AC(1) → arctan2(vol, AC) → 相位差d² × OFIsign | **臨界減速相位加速度**：100-tick區間的價格變化自相關與成交量的相位角二階導，衡量臨界轉變前的減速現象。 |
| 16 | `f_nld_dtb_singularity_cross` | dp_s(EWM span=5)與ddp 3D速度外積 → Z分量(cz=x1·y2−y1·x2) → 加權平均 | **DTB奇點穿越**：價格變化與其二階導數的3D速度外積Z分量（非3D norm），衡量degenerate Takens-Bogdanov奇點附近的穿越行為。 |
| 17 | `f_nld_hopf_eigen_asymmetry` | dp三態轉移矩陣 → 最大實特徵值 × (tm[0,0]−tm[2,2]) | **Hopf特徵非對稱**：價格變化三態轉移矩陣的主特徵值與狀態0/2轉移機率差的乘積，衡量Hopf分岔附近的特徵非對稱性。 |
| 18 | `f_nld_hysteresis_area` | 正規化(dp, dv) 50-tick bin → 有向面積 → 平均 | **滯後環面積**：正規化價格與成交量變化的有向面積，衡量滯後現象的強度。 |
| 19 | `f_nld_limit_cycle_collapse_markov` | dp三態轉移矩陣 → 平穩分佈 · [1, 0, −1] | **極限環崩潰Markov**：價格變化三態轉移矩陣的平穩分佈投影，衡量極限環崩潰後的Markov狀態分佈。 |
| 20 | `f_nld_limit_cycle_radius_accel` | R=sqrt(dp²+dv²) EWM(span=10) → d²R × AsymSign | **極限環半徑加速度**：價格與成交量變化構成的極座標半徑的二階導，衡量極限環的膨脹或收縮加速度。 |
| 21 | `f_nld_lyapunov_divergence_accel` | 延遲嵌入(k=10)距離D EWM(span=10) → d²D × AsymSign | **Lyapunov發散加速度**：價格延遲嵌入空間的軌跡發散度二階導，衡量混沌系統的發散加速度。 |
| 22 | `f_nld_neimark_sacker_torus` | VWAP/dp/dv 3D外積norm → d² × AsymSign | **Neimark-Sacker環面**：VWAP、價格變化、成交量變化構成的3D外積二階導，衡量環面分岔的相位耦合。 |
| 23 | `f_nld_saddle_loop_divergence` | 價格-VWAP穿越間隔 → 二階差分 × 最後偏離sign | **鞍結環發散**：價格圍繞VWAP穿越的時間間隔二階差分，衡量鞍結環附近的發散行為。 |
| 24 | `f_nld_saddle_node_phase_accel` | arctan2(sellPr-price, price-buyPr) EWM(span=10) → d²φ最大值 | **鞍結點相位加速度**：買賣報價相對於成交價的相位角二階導，衡量鞍結點附近的相位加速度。 |
| 25 | `f_nld_stochastic_bifurcation_phase` | 價格雙峰檢測(bins=50, order=3) → VWAP半區間加速度 → mode_vec · accel_vec | **隨機分岔相位**：價格雙峰檢測（50bins, order=3）後，VWAP前半與後半區間加速度與模態向量的內積，衡量隨機分岔的相位特性。 |
| 26 | `f_nld_stochastic_focusing_phase` | arctan2(dp, dv) EWM(span=10) → d²θ × AsymSign | **隨機聚焦相位**：價格與成交量變化的相位角二階導，衡量隨機聚焦的相位特性。 |
| 27 | `f_nld_transient_attractor_proj` | d²Price EWM(span=10) × 吸引子軸距離正規化 | **瞬態吸引子投影**：價格二階導與漲跌停距離正規化的內積，衡量瞬態吸引子的投影強度。 |

---

## 十四、Alpha v14 生態模擬特徵（5 個）
...
```python
# 保留原始 z-score 特徵
df['f_afd_updraft_width_entrainment'] = zscore_rolling(raw, 42).clip(-5, 5)

# 新增橫斷面排名特徵（命名建議加 suffix）
df['f_afd_updraft_width_entrainment_cs_rank'] = df.groupby('Date')['f_afd_updraft_width_entrainment'].rank(pct=True)
```
...
## 二十七、AFD v2 大氣流體力學特徵（交叉券商分點逐筆，15 個，20260331 新增）

**輸入**：券商分點 + 逐筆成交（交叉 `cross_broker_tick`）
**模組**：`features/cross_broker_tick/`
**重要說明**：所有 AFD v2 特徵使用 `eps=1e-10` 參數呼叫 `zscore_rolling`，與原始測試程式碼一致。

| # | 特徵名 | 計算方式 | 概念邏輯 |
|---|--------|---------|---------|
| 1 | `f_afd_moist_cisk_feedback` | det(cov(ret,Top10_NetBuy)) × max(0,Top10-Retail) → log1p → Z(42) | **CISK 反饋**：5日滾動協方差矩陣行列式×大戶淨買超額，捕捉跨週期共振 |
| 2 | `f_afd_diabatic_pv_asymmetric` | max(0,j_buy.diff2)×ret - min(0,j_sell.diff2)×ret → Z(20) | **非絕熱 PV 非對稱**：券商買超二階差分與價格報酬的非對稱組合 |
| 3 | `f_afd_jet_exit_radiation` | max(0,Top5_NetBuy) × ret.diff() → Z(20) | **噴流輻射**：大戶淨買與價格加速度的乘積效應 |
| 4 | `f_afd_edge_wave_phase_locking` | (Top10_5d_mean × Retail_5d_mean) × sign(ret) → log1p → Z(20) | **邊緣波相位鎖定**：大戶與散戶動量共振，同向為正、背離為負 |
| 5 | `f_afd_optimal_growth_hindering` | min(0,buyVol-sellVol) × delta_p × sign(delta_p) → Z(20) | **最佳增長阻礙**：空頭動能被吸收的軋空預測器 |
| 6 | `f_afd_nlsv_energy_extraction` | det(slope_matrix) × max(0,delta_p) → Z(42) | **NLSV 能量提取**：大戶淨買與報酬能量提取矩陣行列式 |
| 7 | `f_afd_entrainment_mass_flux` | max(0,delta_p) × exp(-clip(epsilon, -5, 5)) → Z(42) | **夾帶質量通量**：entrainment rate trigger，指數衰減與價格變化 |
| 8 | `f_afd_dynamic_vppgf_asymmetry` | max(0,large²-small²) × delta_p → log1p → Z(42) | **動態 VP/PGF 非對稱**：大戶動量平方與散戶動量平方的差異 |
| 9 | `f_afd_cold_pool_buoyancy_gradient` | max(0,u_dyn-b_cold) × sign(delta_p) → Z(20) | **冷池浮力梯度**：大戶動力抬升克服散戶負浮力的程度 |
| 10 | `f_afd_goldilocks_cold_pool` | g_c × u_draft × sign(delta_p) → log1p → Z(42) | **適居帶冷池**：適度回檔後噴發的信號 |
| 11 | `f_afd_updraft_width_entrainment` | NB_BuyBrokers × max(0,delta_p) → Z(20) | **上升氣流寬度夾帶**：買方券商數量與正價格變化的乘積 |
| 12 | `f_afd_svc_baroclinic_stretching` | max(0,a_flow.diff2 × omega_s) × max(0,delta_p) → Z(20) | **SVC 斜壓拉伸**：買盤加速度與主動買流向的組合效應 |
| 13 | `f_afd_random_vorticity_stretching` | Jacobian(v_small,v_large) × max(0,delta_p) → Z(42) | **隨機渦度拉伸**：散戶與大戶的拓撲拉伸 Jacobian |
| 14 | `f_afd_surface_drag_convergence` | max(0,spread.diff) × max(0,a_price) → Z(20) | **表面曳力收斂**：價差梯度與價格加速度的正值乘積 |
| 15 | `f_afd_rfd_occlusion_wrap_around` | max(0,sell_z) × buyVol × sign(delta_p) → log1p → Z(42) | **RFD 包覆繞行**：砸盤後正交反轉的信號 |
