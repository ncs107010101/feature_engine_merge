...
| `1e-10` | 極小值特徵 | 適用於 raw 值極小（~1e-9 到 ~1e-12）的特徵，如 AFD 大氣流體力學特徵、NLD 非線性動力學特徵 |
...
```python
# NLD 非線性動力學特徵必須使用 eps=1e-10 (與原始代碼保持一致)
out_series = zscore_rolling(df_res['raw'], window=20, eps=1e-10)
```
