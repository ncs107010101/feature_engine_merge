...
@register_feature
class FSmartAccumulationBlwvwapEwm3TsRank(BaseFeature):
    name = "f_smart_accumulation_blwVWAP_ewm3_ts_rank"
    description = "Profile feature: f_smart_accumulation_blwVWAP_ewm3_ts_rank"
    required_columns = ["StockId", "Date", "r_smart_accumulation_blwVWAP"]
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        ewm = all_features.groupby(level="StockId")["r_smart_accumulation_blwVWAP"].transform(lambda x: x.ewm(span=3).mean())
