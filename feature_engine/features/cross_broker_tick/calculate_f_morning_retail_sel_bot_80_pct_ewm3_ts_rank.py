...
@register_feature
class FMorningRetailSelBot80pctEwm3TsRank(BaseFeature):
    name = "f_morning_retail_sel_bot_80%_ewm3_ts_rank"
    description = "Profile feature: f_morning_retail_sel_bot_80%_ewm3_ts_rank"
    required_columns = ["StockId", "Date", "r_morning_retail_sel_bot_80_pct"]
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        ewm = all_features.groupby(level="StockId")["r_morning_retail_sel_bot_80_pct"].transform(lambda x: x.ewm(span=3).mean())
