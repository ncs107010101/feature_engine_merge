...
@register_feature
class FLargePsellWallTop5pctConcEwm5TsRank(BaseFeature):
    name = "f_large_psell_wall_top_5%_conc_ewm5_ts_rank"
    description = "Profile feature: f_large_psell_wall_top_5%_conc_ewm5_ts_rank"
    required_columns = ["StockId", "Date", "r_large_psell_wall_top_5_pct_conc"]
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        ewm = all_features.groupby(level="StockId")["r_large_psell_wall_top_5_pct_conc"].transform(lambda x: x.ewm(span=5).mean())
