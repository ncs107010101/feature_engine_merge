...
class FeatureAfdSvOptimizationTimeDivergence(BaseFeature):
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        half_idx = df.groupby("Date_int").cumcount() < (df.groupby("Date_int").cumcount().groupby(df["Date_int"]).transform("max") * 0.5)
        
        df["_dir_first"] = np.where(half_idx, df["_direction"], 0.0)
        df["_dir_second"] = np.where(~half_idx, df["_direction"], 0.0)
        
        def compute_divergence(group):
            first = group["_dir_first"].values
            second = group["_dir_second"].values
        ...
        daily_raw = df.groupby(["StockId", "Date_int"]).apply(compute_divergence).reset_index(name="raw")
