...
class FeatureAfdVortexStretchingDot(BaseFeature):
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        df["_vortex"] = df["_direction"] * df["DealCount"]
        
        def compute_stretch(group):
            vortex = group["_vortex"].values
            price_diff = group["_price_diff"].values
            
            if len(vortex) < 10:
                return 0.0
            
            return np.sum(vortex * price_diff)
        
        daily_raw = df.groupby(["StockId", "Date_int"]).apply(compute_stretch).reset_index(name="raw")
