...
class FeatureAfdWaveMeanFlowJacobian(BaseFeature):
    ...
    description = "AFD特徵：波-平均流Jacobian"
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        df["_sec_id"] = df["DealTimeSecond"] // 300
        
        def compute_wave_jacobian(group):
            ...
            mean_flow = np.cumsum(direction)
            wave = price_diff
            
            if len(mean_flow) < 2 or len(wave) < 2:
                return 0.0
            
            J = mean_flow[:-1] * np.diff(wave) - wave[:-1] * np.diff(mean_flow)
            return np.sum(J)
        
        daily_raw = df.groupby(["StockId", "Date_int"]).apply(compute_wave_jacobian).reset_index(name="raw")
