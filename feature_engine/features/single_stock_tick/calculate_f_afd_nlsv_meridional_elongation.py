...
class FeatureAfdNlsvMeridionalElongation(BaseFeature):
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        def compute_elongation(group):
            ...
            J = direction[:-1] * np.diff(price_diff)
            return np.sum(np.abs(J))
        
        daily_raw = df.groupby(["StockId", "Date_int"]).apply(compute_elongation).reset_index(name="raw")
